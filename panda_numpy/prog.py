...
def create_numpy_ones_array(shape):
    ...
    # returns None for shape tuple of values less than zero
    return array
...
def create_identity_numpy_array(order):
    ...
    # returns None for order less than zero
    return array
...
def matrix_cofactor(array):
    ...
    coefmat = array
    array = None
    # return None if the determinant is zero
    if (np.linalg.det(coefmat) != 0):
        array = np.linalg.inv(coefmat).T * np.linalg.det(coefmat)
    return array
...
def f1(X1, coef1, X2, coef2, seed1, seed2, seed3, shape1, shape2):
    ...
    ans = None
    if(X1.shape[0] != shape1[1] or shape1[1] < 0 or shape1[0] < 0):
        return -1
    if(X2.shape[0] != shape2[1] or shape2[1] < 0 or shape2[0] < 0):
        return -1
    ...
    array1 = X1**coef1
    array2 = X2**coef2
    np.random.rand(seed3)
    b = np.random.rand(shape1[0], X1.shape[1])
    ans = np.matmul(W1, array1)+np.matmul(W2, array2)+b
    return ans
