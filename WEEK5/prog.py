...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        grad1 = np.multiply(np.ones(history[1].arr.shape), gradients)
        grad2 = np.multiply(np.ones(history[2].arr.shape), gradients)
        return (grad1, grad2)
    ...
    def grad_matmul(self, gradients=None):
        ...
        """
        history = self.history
        grad1 = np.dot(gradients, np.transpose(history[2].arr))
        grad2 = np.dot(np.transpose(history[1].arr), gradients)
        return (grad1, grad2)
    ...
    def backward(self, gradients=None):
        ...
        """
        history_of_node = self.history
        if self.requires_grad == False:
            return
        if not isinstance(gradients, np.ndarray):
            gradients = np.ones(self.arr.shape)
            gradients = np.squeeze(np.asarray(gradients))
        if history_of_node[0] == 'leaf':
            if self.requires_grad == True:
                self.grad += gradients
        elif history_of_node[0] == 'add':
            grad1, grad2 = self.grad_add(gradients)
            if history_of_node[1]:
                history_of_node[1].backward(grad1)
            if history_of_node[2]:
                history_of_node[2].backward(grad2)
        elif history_of_node[0] == 'matmul':
            g1, g2 = self.grad_matmul(gradients)
            if history_of_node[1]:
                history_of_node[1].backward(g1)
            if history_of_node[2]:
                history_of_node[2].backward(g2)
